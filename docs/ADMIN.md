...
| Users | `/admin/users` | Live, read/write | Roster + per-user detail (subscription + tier, linked marketplace accounts, devices, listings + storage, usage vs caps for a selectable period (this period / last 30 days / all time; caps and percent only on the current-period view), ticket count, admin badge). See "User observability" below. The detail drawer can edit the user's subscription (tier / version / status) via `admin_set_user_subscription` (audit-logged), change a customer's paid plan in Stripe via the `admin-change-plan` Edge Function (step-up reauth, audit-logged), and delete the account via the `admin-delete-user` Edge Function (step-up reauth, audit-logged, GDPR runbook). |
...
| Extension usage | `/admin/usage` | Live, read-only | Extension feature usage for a selectable period (current period by default; last 5 / 15 / 30 minutes, last 1 / 2 / 12 / 24 / 48 / 72 hours, last 7 / 30 days, all time, or a custom from/to range of UTC hours via `?range=` / `?from=&to=` URL params; the resolver also still accepts day-shaped `?from=YYYY-MM-DD&to=YYYY-MM-DD` for old links, but the picker only offers hours), grouped by user: one collapsible card per user whose body lists the FULL feature roster (`lib/admin/extension-features.ts`) with counts, zero included. On the current-period view the five tier-metered verbs (crosslist, relist, refresh, follow, unfollow) also show their `tier_limits` cap and percent (near-limit badge on the collapsed header); range views drop caps/percent because caps are per-period. Day ranges sum day buckets plus every month bucket touched, and monthly counters only exist as month buckets, so a range partially covering a month includes that whole month's count for them. Trailing presets (`?range=` with one of `5m`, `15m`, `30m`, `1h`, `2h`, `12h`, `24h`, `48h`, `72h`) are exact windows ending now, answered from `usage_events` (migration 017; see "Usage events" below), so "last hour" at 15:42 is 14:42 to 15:42. Custom hour ranges (`?from=YYYY-MM-DDTHH&to=YYYY-MM-DDTHH`, UTC) sum the server-derived hour buckets only, which are exact for every counter but exist only from `HOUR_EPOCH` and for `HOUR_RETENTION_DAYS` (60 days) in `lib/admin/period.ts`; a request outside that window is clamped and the header shows a "(clamped)" marker with the reason. See "Hour buckets" below. The rest are uncapped activity counters recorded by the extension (salelinx-app `src/entitlements/usage-tracking.ts`). Users sorted by total actions. |
| Analytics dashboard | `/admin/analytics` | Live, read-only | Landing page for the Analytics sidebar group: a grid of one-glance boxes fixed to the current month (last month as comparison), each linking to the module with the detail. Boxes: Top features (three most used by distinct users, with share of active users, actions, change vs last month), Top users (three most active accounts by total actions, with tier, features touched and top feature; emails resolved for just those ids), Active users (six-month column chart of distinct active users, `components/admin/analytics/ColumnChart.tsx`, with delta vs last month and the prior three-month average), Movers (largest gains and drops in users per feature vs last month, `featureMovers` in `lib/admin/adoption.ts`), Churn watch (paying users with `cancel_at_period_end` set, and paying users with no extension activity for 30+ days judged by `usage_counters.updated_at`; `lib/admin/retention.ts`, tested in `tests/retention.test.ts`), Signup funnel (accounts, linked a shop, ran an action, paying, each as a share of all accounts), Marketplace health (broken / degraded feature counts from the same `loadHealthRows(24)` the health module uses). The page reads usage rows once via `loadAnalyticsSource` and folds them for every usage-derived box; it is `force-dynamic`. Add a box by loading its data in the page's `Promise.all` and dropping a component into the grid (`components/admin/analytics/AdminAnalyticsDashboard.tsx`). |
| Feature adoption | `/admin/usage/features` | Live, read-only | The per-feature cut of the same extension counters. Three kinds of window: a run of whole months (one / 3 / 6 / all time via `?months=`, anchored by `?to=YYYY-MM`), the same exact trailing presets as the usage pages (`?range=5m` to `72h`, read from `usage_events`), or a custom UTC hour range (`?from=YYYY-MM-DDTHH&to=YYYY-MM-DDTHH`, read from the hour buckets). In every kind each of the 28 roster features shows distinct users, share of a selectable base (`?base=active` users with any extension activity, `paid`, or `all` accounts), change in users against the preceding window of equal length, actions, median actions per user, and a 6-month sparkline of users. A second table splits adoption by plan, greying out cells the plan cannot use (via the counter-to-gate map in `lib/admin/adoption.ts`). The sparkline is per month for month windows and per hour for hour ranges; trailing presets have no per-bucket breakdown, so it is hidden for them. Months before 26 Aug 2026 are flagged as unmeasured for the activity counters. Windows reaching before the hour or event epochs are clamped with a "(clamped)" marker. See "Feature adoption" below. |
| Web usage | `/admin/usage/web` | Live, read-only | Web abuse rate limits for a selectable period (same `?range=` / `?from=&to=` params as Extension usage, including hour resolution) covering checkout / portal sessions, deletion requests, label emails, email changes. On the current-period view counts are measured against the hardcoded per-day cap in the calling code; range views show plain totals (the limits are per-day, so a multi-day sum has no meaningful percent). |
...
| Analytics dashboard | `app/admin/analytics/page.tsx` + `components/admin/analytics/AdminAnalyticsDashboard.tsx` (boxes live in that file until one outgrows it) |
...
1. **Edge gate (`proxy.ts`).** For `/admin/*` the middleware verifies the JWT locally with `getClaims()` (ES256 signing keys, no network round-trip), then checks `admin_users` membership via the self-read policy. Signed out, not an admin, or any error -> **404**, served by rewriting the request into the `[locale]` tree's `[...rest]` catch-all (locale from the `NEXT_LOCALE` cookie, `en` otherwise) so the visitor sees the site's ordinary not-found page, exactly as for any unknown URL. An admin whose session is still AAL1 -> redirect to `/auth/mfa?next=...`. A non-admin never reaches admin route code.
2. **Layout gate (`app/admin/layout.tsx`).** Re-runs the same checks, fail-closed, so a missed matcher or bypassed middleware still denies: signed out, not an admin, or any error -> `notFound()`; admin at AAL1 -> `redirect("/auth/mfa?next=/admin")` (from `next/navigation`, NOT the localized `@/i18n/navigation`). The layout is a root layout with no not-found boundary above it, so its `notFound()` renders Next's built-in 404 rather than the localized page; it only fires if Layer 1 was bypassed, so that is acceptable.

**Why 404 and not a redirect.** The console is unlinked from the public site and reachable only by typing the URL. A redirect to `/auth/login` or `/account` would confirm that `/admin` exists and is protected; a 404 makes it indistinguishable from a typo. This means an admin who is signed out also sees a 404 on `/admin`: sign in at `/auth/login` first, then return. Nothing about the response (status, body, timing beyond one `admin_users` read) differs between "signed out" and "signed in but not an admin".
...
- `admin_list_usage(period_keys[])` - `usage_counters` rows for the passed period keys. The keys are always generated server-side from a bounded date range (default: the current period; widest: all time since `USAGE_EPOCH` in `lib/admin/period.ts` at day resolution, or the last `HOUR_RETENTION_DAYS` at hour resolution), so the read stays bounded (the table grows ~ users x features x periods; daily rows accumulate one per user per day, hour rows one per user per feature per active hour for 60 days). If the base grows, add pagination / a top-N cap, and consider a from/to variant of the RPC instead of enumerating day or hour keys.
- `admin_list_usage_events(since, until)` - per (user, feature) totals from `usage_events` between two timestamps (window capped at 8 days), for the trailing-window presets. Same answer shape as `admin_list_usage` so `loadUsageRows` treats the two sources alike.
...
Migration `016_usage_hour_buckets.sql` makes `increment_usage_counter` upsert a second row on every call, keyed `YYYY-MM-DDTHH` (UTC) and derived server-side from `now()`, next to the caller-named month or day bucket. This is what lets the usage pages narrow a range below a day: month and day rows are running totals with no record of when within the period an action happened. Things to keep straight:

- **Hour ranges sum hour keys only.** The hour bucket nests inside the day and month buckets, so mixing them counts the same action twice. `hourKeysForRange` never emits a day or month key, and `resolveUsageRange` never mixes resolutions (a `from`/`to` pair of mismatched shapes falls back to the current period).
- **Nothing else reads hour rows.** The extension's usage meters, `/account`, the users drawer, adoption and analytics all select by explicit month or day keys, so the extra rows are invisible to them. Keep it that way: any new consumer that reads `usage_counters` without a key filter must exclude `HOUR_KEY_RE` rows.
- **Hour rows are short-lived and separately capped.** `purge_stale_usage_counters` drops them after 60 days (month/day rows keep 14 months); they are excluded from the 2000-row per-user cap and have their own soft cap of 10000, at which the hour write is skipped rather than raised so the primary counter is never blocked.
- **`HOUR_EPOCH` is the moment 016 went live.** Earlier hours have no rows and would read as 0, so the resolver clamps to it and says so. Update the constant if the migration is applied on a different day than the one recorded.
- **Minutes are not buckets.** Anything finer than an hour is answered from `usage_events`, not from counters; see "Usage events" below.

### Usage events

Migration `017_usage_events.sql` adds `usage_events`: one row per `increment_usage_counter` call (user, feature, the call's delta, server timestamp), written by the function itself in the same call. It exists for the trailing-window presets (last 5 minutes to last 72 hours), which the bucketed counters cannot answer: a bucket says how much, never when. Things to keep straight:

- **Windows are exact and read events only.** `resolveUsageRange` returns `period.window` (`since` / `until` ISO timestamps) with an empty `keys` list, and `loadUsageRows` calls `admin_list_usage_events` instead of `admin_list_usage`. Never combine the two sources for one view.
- **One event per call, not per item.** A bulk refresh of 200 items is a single event with `delta = 200`, so volume tracks feature runs, not list sizes.
- **Short-lived, separately guarded.** Rows are purged after 7 days (hourly `purge_stale_usage_events` cron); per user, at most 2000 events per rolling hour are recorded, after which the event insert is skipped (never raised) so the primary counter is unaffected. The admin RPC refuses windows over 8 days.
- **`EVENTS_EPOCH` is the moment 017 went live** (`lib/admin/period.ts`). The resolver clamps windows to it and to the retention floor, and the header shows "(clamped)" with the reason.
- **It is personal data.** `user_id` plus a timestamp per action: listed in `docs/GDPR.md`, cascades on account deletion, no client policies (only the SECURITY DEFINER functions touch it).
...
- **Three window kinds, one fold.** `AdoptionWindow.kind` says what the
  buckets are. `months`: runs of whole months (`?months=1|3|6|all`, anchored
  by `?to=YYYY-MM`); the 23 activity counters are month buckets and the daily
  verbs are summed into their month, so a day picker would promise precision
  the data does not have. `hours`: a custom UTC hour range
  (`?from=&to=` as hour keys) over the hour buckets from migration 016, with
  a per-hour sparkline. `window`: the trailing presets shared with the usage
  pages (`?range=5m` to `72h`), exact windows read from `usage_events`
  (migration 017) through `admin_list_usage_events`; the loader reshapes the
  totals into two synthetic buckets, `window` and `compare`, so the fold is
  unchanged. The fold treats `months` / `compareMonths` / `trendMonths` as
  opaque bucket keys (the names are historical); only the `months` kind
  slices `period_key` to a month. Never mix sources within one window.
- **The comparison window is the preceding run of equal length**, and is
  dropped entirely (no deltas) when it would reach past the span that has data
  (`USAGE_EPOCH` for months, `HOUR_EPOCH` / hour retention for hour ranges,
  `EVENTS_EPOCH` / event retention for trailing presets): a shorter baseline
  would make every delta look like growth. Events are kept 7 days, so the
  72-hour preset plus its comparison (6 days) is the longest events window.
