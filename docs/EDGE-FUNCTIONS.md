...
- **`send-shipping-labels`** - called by the Chrome extension to email a merged shipping-label PDF via Resend. Lives here (not in the extension) because Edge Function deploys are co-located with everything else Supabase-adjacent, and the extension only needs the function URL. The tier gate (`features.shipping_label_email`) exempts `admin_users` members: the extension forces every feature on for admins with no tier preview set, so without the exemption the button showed unlocked and the send came back 403. The daily cap still applies to admins.
...
- **`change-plan`** - the self-serve counterpart to `admin-change-plan`. A trialing user is already a live Stripe subscription, so `create-checkout-session` refuses them with 409 and the trial cap had no release valve: someone who burned their 50 crosslists on day 3 sat at 50 until day 14 or churned. This ends the trial (`trial_end: "now"`) and optionally swaps the price, in one Stripe call. **It charges the card immediately** - the caller must confirm with the user first. Deliberately up-only: downgrades and cancellations stay in the Customer Portal, where Stripe owns the proration and refund wording. Never writes `tier_id`/`status`; `stripe-webhook` syncs the row. Rate limited to 10/day per caller.
...
| `send-shipping-labels`    | false*       | Extension POSTs a base64 PDF + recipient; auth by `getUser(jwt)` + tier gate (admins exempt) + daily cap; via Resend |
...
| `change-plan`             | false*       | Trialing user ends their trial early and/or moves up a tier; auth by `getUser()`, own subscription only |
