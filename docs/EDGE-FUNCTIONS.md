...
- **Per-user rate limits** (all via `increment_usage_counter`, keyed on the caller's auth.uid): `send-shipping-labels` 15/day, `delete-account` request stage 5/day, `create-checkout-session` 20/day, `create-portal-session` 20/day, account email-change 5/day. Support ticket and reply creation are capped by DB triggers instead (migration 014 tickets, `015_rate_limit_gaps` replies); `listings` and `linked_accounts` by row-count triggers (017).
