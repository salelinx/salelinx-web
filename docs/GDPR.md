...
| Device sessions | Random per-install device ID, user agent, extension version (migration 041), last-seen timestamps | Users | Legitimate interest (enforcing the per-plan concurrent-device cap; knowing which build a support report came from) | Supabase `device_sessions` | Rows idle 30+ days pruned on next claim; cascades on account deletion |
