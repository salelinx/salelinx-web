...
| Extension settings | Bot timing preferences, marketplace preference, and user-named settings presets (numeric caps, delays, toggles and the names the user gives them; `user_settings.preferences`, no schema change; presets are only uploaded on plans with the `preset_sync` feature, 018_preset_sync_feature.sql). Contains no buyer, listing or message content | Users | Contract (settings that follow the account across devices) | Supabase `user_settings` | Life of account (FK cascades) |
...
| Device sessions | Random per-install device ID, user agent, extension version (015_device_session_version.sql), last-seen timestamps | Users | Legitimate interest (enforcing the per-plan concurrent-device cap; knowing which build a support report came from) | Supabase `device_sessions` | Rows idle 30+ days pruned on next claim; cascades on account deletion |
