...
- `delete-account` - `verify_jwt = false` (self-serve GDPR deletion from the `/account` Danger zone; handler validates via `getUser()` and deletes the caller's own account with the same steps as `admin-delete-user`; refuses admins; see `docs/GDPR.md`)
