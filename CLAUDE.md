...
- `resolve-category` - `verify_jwt = false` (handler validates via `getUser(jwt)`, checks the caller's tier allows crosslisting and has monthly allowance left, then resolves against the tables in `_generated/`. **Live: the extension calls this on every crosslist**, from `src/features/crosslist/category-resolve-client.ts`, with no local fallback - so if this function is down, crosslisting is down. That is deliberate; a fallback would make the cap bypassable. The extension still ships its own copies of the tables, but nothing reads them at runtime. See `docs/EDGE-FUNCTIONS.md` "Wiring up resolve-category" for what is left)
- `change-plan` - `verify_jwt = false` (self-serve plan change: a trialing user ends their trial early and/or moves UP a tier. Handler validates via `getUser()` and only ever touches the caller's own subscription - there is no userId param, which is the difference from `admin-change-plan`. Charges the card immediately, so the UI confirms first; up-only, downgrades go to the Customer Portal)
...
- **Hour-resolution usage ranges sum hour keys only.** `increment_usage_counter` writes a server-derived `YYYY-MM-DDTHH` bucket next to the caller's month/day bucket (migration 016), so an hour bucket nests inside a day bucket; summing both counts an action twice. `resolveUsageRange` never mixes resolutions, and anything new that reads `usage_counters` without an explicit key filter must exclude `HOUR_KEY_RE` rows. Hour rows only exist from `HOUR_EPOCH` and for 60 days. The trailing presets (last 5 minutes to last 72 hours) do not use buckets at all: they read `usage_events` (migration 017, one row per RPC call written by the same function, 7-day retention) through `admin_list_usage_events`. Never mix the two sources in one view. See `docs/ADMIN.md` "Hour buckets" and "Usage events".
...
- **Google Analytics and the Google Ads conversion tag must only ever load through `components/CookieConsent.tsx`** - PECR requires prior consent, so never add a gtag/GTM `<script>` to a layout. Consent is per category: analytics is gated on `NEXT_PUBLIC_GA_MEASUREMENT_ID`, ad measurement on `NEXT_PUBLIC_GOOGLE_ADS_ID`; blank env var means that category (and with both blank, the whole banner) does not exist. The per-category choice lives in the `slx_consent` cookie (6 months, `analytics=...&ads=...` format, legacy bare `granted`/`denied` parses as analytics-only - see `lib/consent.ts`); the footer Cookie settings button reopens the banner via the `slx:cookie-settings` window event. The Ads tag exists because we advertise SaleLinx on Google - we never show ads on the site - and is conversion-measurement only: `ad_personalization` stays denied even with ads consent. Do not flip it without a new consent category plus privacy-policy and `docs/GDPR.md` updates. Conversion events live in `lib/tracking.ts` and are safe to call anywhere client-side (no-ops without consent).
