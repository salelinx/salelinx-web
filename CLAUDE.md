...
- **The theme must never be stored in a cookie.** The privacy policy's Cookies section publishes it as local-storage-only ("never sent to us"). The boot path is: inline pre-paint script in `app/[locale]/layout.tsx` (reload persistence) + a pathname-keyed effect in `ThemeToggle.tsx` (locale switches reconcile `<html>` className and wipe the `dark` class - commit 7523761 history). Reintroducing a theme cookie requires a privacy-policy cookie-inventory update and a new `LAST_UPDATED` in the same PR.
